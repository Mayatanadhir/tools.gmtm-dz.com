    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3.5">
                <x-tool-icon name="calibrator-movements" class="w-11 h-11 sm:w-12 sm:h-12 shrink-0" />
                <div>
                    <h2 class="font-bold text-2xl text-gray-900 dark:text-white leading-tight">
                        {{ __('Calibrator Movements') }}
                    </h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                        {{ __('Track movements, transfers, and field assignments of calibration devices') }}
                    </p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                @can('create calibrator movements')
        ...
        <div class="w-full px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col lg:flex-row gap-6 items-start">
                ...
                <aside class="w-full lg:w-64 shrink-0">
                    <x-metrology-tabs active="calibrator-movements" />
                </aside>
                    ...
                    <x-table>
                        <x-slot:toolbar>
                            <h3 class="text-base font-bold text-gray-900 dark:text-white flex items-center gap-2.5">
                                <x-tool-icon name="calibrator-movements" class="w-6 h-6 shrink-0" />
                                <span>{{ __('Calibrator Movements') }}</span>
                            </h3>
                        </x-slot:toolbar>
