<x-app-layout>
    ...
    <div class="py-8" x-data="{
        points: {{ Js::from($certificate->calibrationPoints->map(fn($p) => [
            'nominal_value' => $p->nominal_value,
            'correction' => $p->correction,
            'uncertainty' => $p->uncertainty,
            'equipment_specification_id' => $p->equipment_specification_id,
        ])->values()) }},
        calDate: '{{ $certificate->calibration_date ? $certificate->calibration_date->format('Y-m-d') : '' }}',
        validityMonths: {{ $certificate->validity_period_months ?: 12 }},
        expiryDate: '{{ $certificate->expiry_date ? $certificate->expiry_date->format('Y-m-d') : '' }}',
        updateExpiry() {
                        ...
                        <div>
                            ...
                            @if($certificate->certificate_path)
                                <p class="text-[11px] text-emerald-600 dark:text-emerald-400 mt-1">
                                    <i class="fas fa-check-circle mr-0.5"></i> {{ __('Current document') }}: {{ $certificate->file_name ?: basename($certificate->certificate_path) }}
                                </p>
                            @endif
                        </div>
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200/80 dark:border-gray-700/80 p-6 shadow-sm space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" step="0.1" name="environmental_conditions[temperature_celsius]" value="{{ $certificate->environmental_conditions['temperature_celsius'] ?? '' }}" class="w-full text-xs rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-gray-900 dark:text-white">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Relative Humidity (%)') }}</label>
                            <input type="number" step="0.1" min="0" max="100" name="environmental_conditions[humidity_percent]" value="{{ $certificate->environmental_conditions['humidity_percent'] ?? '' }}" class="w-full text-xs rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-gray-900 dark:text-white">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Atmospheric Pressure (hPa)') }}</label>
                            <input type="number" step="0.1" name="environmental_conditions[atmospheric_pressure_hpa]" value="{{ $certificate->environmental_conditions['atmospheric_pressure_hpa'] ?? '' }}" class="w-full text-xs rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-gray-900 dark:text-white">
                        </div>
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Metrologist Remarks') }}</label>
                        <textarea name="remarks" rows="2" class="w-full text-xs rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-gray-900 dark:text-white">{{ $certificate->remarks }}</textarea>
                    </div>
                </div>

                <!-- Section 3: Calibration Points Grid -->
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200/80 dark:border-gray-700/80 p-6 shadow-sm space-y-4">
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                <template x-for="(pt, idx) in points" :key="idx">
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                                        ...
                                        <td class="py-2 px-3">
                                            <input type="number" step="any" :name="'points[' + idx + '][nominal_value]'" x-model="pt.nominal_value" required class="w-full text-xs rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-gray-900 dark:text-white">
                                        </td>
                                        <td class="py-2 px-3">
                                            <input type="number" step="any" :name="'points[' + idx + '][correction]'" x-model="pt.correction" required class="w-full text-xs rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-gray-900 dark:text-white">
                                        </td>
                                        <td class="py-2 px-3">
                                            <input type="number" step="any" min="0" :name="'points[' + idx + '][uncertainty]'" x-model="pt.uncertainty" required class="w-full text-xs rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 text-gray-900 dark:text-white">
                                        </td>
